...
import { DisplayAsset } from '@/api/type';
...
export const MyRuneList = ({ baseRuneSummary }: RuneListProps) => {
  ...
  const  value  = useMemo(() => {
    const ret : { runelist: DisplayAsset[], total: number } = {
      runelist: [],
      total: 0,
    }
    for (const runeSummary of baseRuneSummary) {
      ...
      ret.runelist.push(runeSummary);
      ret.total += Number(runeSummary.Amount);
    }
    return ret
  },[baseRuneSummary]);

  const getRuneList = async (ticker: string) => {
    const param = { Protocol: "runes", Type: "f", Ticker: ticker }
    const data = await indexer.runes.getRune(param)
    let sum = 0;
    if (data.code !== 0) {
      sum = 0;
    }
  ...
  const list = useMemo(() => value?.runelist || [], [value]);
