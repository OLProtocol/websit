...
git checkout test && git merge main && npm run build:test && git add . && git commit -m "publish: generate test dist" && git push
git checkout dev && git merge main && npm run build:dev && git add . && git commit -m "publish: generate dev dist" && git push
